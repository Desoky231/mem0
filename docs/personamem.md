...
uv run python -m mem0_eval.run personamem --backend graph --dry-run
```
...
| Metric | Text v2.0.14 | Historical graph v0.1.45 |
|---|---:|---:|
...
This is a historical diagnostic sample, not a result from the new composite
Mem0g backend and not a full PersonaMem-v2 reproduction. See the
[full report](reports/mem0_paired_baseline_evaluation.html) for confidence
